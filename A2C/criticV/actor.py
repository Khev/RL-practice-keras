...
class Actor:
    ...
    def optimizer(self):
        ...
        R_pl = K.placeholder(name='R', shape=(None,1))
        V_pl = K.placeholder(name='V',shape=(None,1))  
        V1_pl = K.placeholder(name='V1',shape=(None,1)) 

        #Find advantage term (which multiplyies the grad log pi term)
        Adv = R_pl + self.gamma*V1_pl - V_pl
        
        #Find grad log( pi)
        pi_pl = self.model.output
        pars = self.model.trainable_weights
        grads = tf.gradients(K.log(pi_pl), pars, -Adv)   #scalar multiply by the adv
        grads = [K.mean(grad) for grad in grads]
        ...
        return K.function(inputs = [state_pl, R_pl, V_pl, V1_pl], outputs = [], updates = updates)
